...
def _local_classify(message: str) -> dict:
    ...
    if re.search(r"simply|like i'?m a (beginner|kid)|eli5|no jargon|basic", m):
        ...
        domain_terms = r'eigenvalue|fourier|hamiltonian|manifold|entropy|thermodynamic|quantum|gradient|topology|differential equation'
...
async def classify_intent(message: str, conversation_history: list = None) -> dict:
    ...
    try:
        ...
        result = json.loads(raw) if raw else _local_classify(message)
    except Exception:
        result = _local_classify(message)
    ...
    result["depth_label"] = ["", "beginner", "intermediate", "advanced"][result["depth"]]
    if result["intent"] == "challenge":
        subtype = result.get("challenge_subtype", "general")
        result["challenge_subtype"] = subtype if subtype in CHALLENGE_SUBTYPES else "general"
    else:
